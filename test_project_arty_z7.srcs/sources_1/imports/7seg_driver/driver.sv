...
always_ff @(posedge clk_i or negedge porb_i) begin
    if (!porb_i) begin
        for (int i = 0; i < 8; i++) 
            digits_ff[i] <= 8'd0;
    end else if (disp_strobe_i) begin
        for (int i = 0; i < 8; i++) 
            digits_ff[i] <= digits_i[i];
    end
end
        ...
        SEND_ADDR_MODE_CMD:
            begin
                ...
                if (fsm_next == SEND_ADDR_AND_DIGITS) begin
                    ...
                    seg_data[1] = digits_ff[0];
                    seg_data[2] = digits_ff[1];
                    seg_data[3] = digits_ff[2];
                    seg_data[4] = digits_ff[3];
                    seg_start = 'b1;
                end
            end
        SEND_ADDR_AND_DIGITS:
